...
**Деньги**
- Счета в любых валютах: карты, наличные, накопления, вклады и золото в граммах
  (в итог идёт по цене грамма). Любой счёт можно исключить из общего баланса.
...
- Два курса у валюты — банковский и рыночный, итог считается по выбранному.
  Банковский подтягивается одной кнопкой из ЦБ России, Нацбанка Казахстана или ЦБ Узбекистана.
...
- Регулярные платежи: кредиты, рассрочки, подписки, аренда — с напоминанием накануне
  и счётчиком «оплачено 5 из 12».
- Обмен через CSV: выгрузка за неделю, месяц, квартал, год или целиком и загрузка по шаблону
  (неизвестные счета и категории создаются сами). Импорт выписки банка: колонки узнаются
  по заголовкам, уже записанное не повторяется.
...
**Без ручного ввода**
- Чек по QR-коду — без интернета и без ИИ: сумма и дата прямо из кода фискального чека.
- Чеки по фото: ИИ-провайдер по вашему ключу читает магазин, дату, сумму и позиции.
- Банковские СМС (сборка с GitHub): правила по отправителю, привязка к счёту и последним
  4 цифрам карты, готовые шаблоны банков Туркменистана, проверка правила на настоящей смске,
  разбор истории за 90 дней.
- Уведомления банковских приложений (Android) — для банков, которые не шлют СМС.
- Всё это попадает «На проверку» — в деньги ничего не пишется без вашего подтверждения.
...
- Пять языков: русский, английский, туркменский, узбекский (латиница) и казахский.
  По умолчанию — как в системе, переключается в настройках.
...
- Копия файлом — без Google и интернета: сохранить на телефон или отправить себе
  в мессенджер и восстановить на любом телефоне.
- ИИ-советник работает по вашему ключу: Claude, OpenAI, Gemini, GigaChat, YandexGPT или свой endpoint.
...
shared/src/commonMain/    общий код — данные, расчёты, языки, сеть и весь интерфейс
  ...
  net/     Ai.kt (Claude, OpenAI, Gemini, GigaChat, YandexGPT, свой endpoint) · DriveApi.kt · Http.kt
